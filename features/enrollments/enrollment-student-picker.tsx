...
import { ClassStudentsInput } from "./schema"

export function StudentSelectAllCheckbox({
  table,
}: {
  table: ReactTable<Student>
}) {
  const form = useFormContext<ClassStudentsInput>()
...
const columns: ColumnDef<Student>[] = [
  {
    id: "select",
    header: ({ table }) => <StudentSelectAllCheckbox table={table} />,
    cell: ({ row }) => <StudentRowCheckbox row={row} />,
    enableSorting: false,
    enableHiding: false,
    size: 32,
  },
  ...
  },
  {
    ...
    accessorKey: "gender",
    cell: ({ row }) => (
      <Badge variant={row.original.gender}>
...
export function EnrollmentStudentsPicker({
  students,
  isLoading,
}: {
  students: Student[]
