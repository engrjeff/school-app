...
interface DetailedFaculty extends Faculty {
  programOffering: ProgramOffering
}

const columns: ColumnDef<DetailedFaculty>[] = [
  {
    header: "Name",
    accessorKey: "title",
    cell: ({ row }) => (
      <p className="line-clamp-1 group-hover:underline">{row.original.title}</p>
    ),
  },
  {
    header: "Program",
    accessorKey: "programOffering.title",
    cell: ({ row }) => (
      <>
        <p className="line-clamp-1">{row.original.programOffering.title}</p>
        <p className="text-muted-foreground text-xs">
          {row.original.programOffering.code}
        </p>
      </>
    ),
  },
...
export function TeacherFacultiesTable({
  faculties,
}: {
  faculties: DetailedFaculty[]
}) {
  ...
  return (
    <div className="space-y-4 overflow-hidden">
      ...
      {/* Pagination */}
      {pagination.pageSize > faculties.length ? null : (
