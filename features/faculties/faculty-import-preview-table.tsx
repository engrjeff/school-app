...
import { FacultyInputs } from "./schema"

const columns: ColumnDef<FacultyInputs>[] = [
  {
    header: "Row",
    cell: ({ row }) => (
      <div className="font-mono font-medium">{row.index + 1}</div>
    ),
    id: "row",
    size: 40,
  },
...
export function FacultyImportPreviewTable({
  facultyPreviewData,
}: {
  facultyPreviewData: FacultyInputs[]
}) {
  const id = useId()
      ...
      <div className="bg-accent rounded border border-l-2 border-l-emerald-500 px-4 py-3">
        <p className="text-sm">
          <CircleCheck
            ...
            aria-hidden="true"
          />
          Parsed {facultyPreviewData.length} rows.
        </p>
      </div>
