...
>(({ className, containerClass, ...props }, ref) => (
  <div
    className={cn(
      'relative flex-1 w-full overflow-auto bg-muted/80 h-full',
      containerClass
    )}
  >
    <table
      ref={ref}
      className={cn('w-full caption-bottom text-sm mb-4', className)}
      {...props}
    />
  </div>
));
...
>(({ className, ...props }, ref) => (
  <thead
    ref={ref}
    className={cn('[&_tr]:border-b bg-neutral-900', className)}
    {...props}
  />
));
